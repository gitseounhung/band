...
const displayCreateAt = (createdAt) => {
  ...
  // 변환 (asSeconds 를 하면 오늘과의 시간차이를 초단위로 float datatype 으로 보여준다 (3.82 이런식)
  ...
  } else if (weeks < 1) { // 일주일 미만이면 일 단위로 보여주고
    return parseInt(days) + '일 전'
  } else if (month < 1) { // 한 달 미만이면 주 단위로 보여주고
    return parseInt(weeks) + '주 전'
  } else if (year < 1) { // 1년 미만이면 달 단위로 보여주고
    return parseInt(month) + '달 전'
  } else { // 1년 이상이면 넌 단위로 보여주고
